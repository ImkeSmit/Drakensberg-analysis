...
####Some descriptive figures to aid interpretation####
cwm_subs <- cwm_H |>  bind_rows(cwm_SLA) |> 
  dplyr::select(Cell_ID, trait, cwm_value, zrock_cover, znorthness, zsoil_moist, zsoil_depth, zslope_height) |> 
  pivot_longer(cols = !c(Cell_ID, trait, cwm_value), names_to = "env_variable", values_to = "value")

ggplot(cwm_subs, aes(x = value, y = cwm_value)) +
  geom_point()+
  facet_grid(trait ~ env_variable, scales = "free")
