...
#first date after June 21 where all logers at a site had a min  temp above 5
start_grw <- min_daily_temp |>
  ...
  summarise(all_above_5 = all(min_daily_T1 > 5),    # TRUE only if every Cell_ID > 5
            .groups = "drop") |> 
  filter(all_above_5) |> #only keep dates where all loggers had min temps above 5
  group_by(site) |> 
  slice_min(date, n = 1) |>   #earliest date per site
  rename(start_grw_date = date) |> 
  dplyr::select(!all_above_5)

##latest date before 21 June where all loggers at a site had a min temp above 5
