...
#This is based on euclidean distance. 
...
#Unweighted RaoQ
calc_RaoQ_unweighted <- function(mean_traits, abun_matrix) {
...
##Abundance weighted RaoQ
calc_RaoQ_weighted <- function(mean_traits, abun_matrix) {
  ...
  for(t in 1:length(traitlist)) {
    ...
    #calculate RaoQ 
    FD_cells <- dbFD(chosen_trait, abun_temp,
                     w.abun = T, #weight RaoQ by abundances
                     corr = "cailliez", 
                     calc.FRic = F, 
                     scale.RaoQ = F, 
                     calc.FGR = F, 
